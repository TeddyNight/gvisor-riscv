...
//
//go:nosplit
func fpsimdPtr(addr *byte) *arch.FpregsContext {
	return (*arch.FpregsContext)(unsafe.Pointer(addr))
}

// dieArchSetup initialies the state for dieTrampoline.
//
// The riscv64 dieTrampoline requires the vCPU to be set in R1, and the last PC
...
//
//go:nosplit
func bluepillArchFpContext(context unsafe.Pointer) *arch.FpregsContext {
